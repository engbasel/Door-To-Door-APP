import 'package:flutter/material.dart';

class AppColors {
  // Primary Colors
  static const Color primaryYellow = Color(0xFFFDC500);
  static const Color secondaryYellow = Color(0xFFFDD500);
  static const Color primaryBlue = Color(0xFF00509D);
  static const Color secondaryBlue = Color(0xFF003F88);
  static const Color darkBlue = Color(0xFF00296B);

  // Light Theme Colors
  static const Color lightBackground = Color(0xFFF8F9FA);
  static const Color lightSurface = Color(0xFFFFFFFF);
  static const Color lightTextPrimary = Color(0xFF212529);
  static const Color lightTextSecondary = Color(0xFF495057);

  // Dark Theme Colors
  static const Color darkBackgroundblack = Color(0xFF121212);
  static const Color darkSurface = Color(0xFF1E1E1E);
  static const Color darkTextPrimary = Color(0xFFEAEAEA);
  static const Color darkTextSecondary = Color(0xFFB0B0B0);

  // Accent Colors
  static const Color success = Color(0xFF28A745);
  static const Color warning = Color(0xFFFFC107);
  static const Color danger = Color(0xFFDC3545);
  static const Color info = Color(0xFF17A2B8);
}
